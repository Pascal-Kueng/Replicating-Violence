...
The analysis sample consists of $N=82$ site-period observations (34 zero-trauma cases). To implement the beta-binomial model, continuous proportions were reconstructed into integer counts, ensuring the approximation is small (mean absolute discrepancy 0.021 counts), so the reconstructed counts closely track the original shares.
...
| **Regionms** | Geography (Mesopotamia) | **Sig** ($p=.005$) | Borderline ($p=.051$) | **Sensitive to model** |
| **Regionl** | Geography (Levant) | NS ($p=.165$) | **Sig** ($p=.030$) | **New Finding** |
| **Regiontr** | Geography (Turkey) | NS ($p=.079$)	| NS (p=.653) | **Robust** |
...
**C. New Insights:**
* **Regional Heterogeneity:** The model choice flips the significance of regional fixed effects. In the beta-binomial model, Regionl (Levant) becomes a significant negative predictor ($p=0.030$), indicating that violence levels in the Levant (~10.8% in raw data) were significantly lower than in the reference region of Iran (~18.6%). This distinction was obscured in the Tobit model ($p=.165$).
